...
const DigitalClock: React.FC = () => {
    ...
    return (
        <div className="text-right hidden xl:block">
            <div className="font-mono text-lg font-bold text-gray-800 dark:text-gray-100 tracking-wider leading-none">
                {formatTime(currentTime)}
            </div>
            <div className="text-[10px] uppercase font-bold tracking-widest text-gray-400 dark:text-gray-500 mt-1">
...
const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className="w-full py-4 px-4 md:px-8 glass sticky top-0 z-50 border-b-0 shadow-lg transition-all duration-300">
      <div className="flex items-center justify-between max-w-[1920px] mx-auto">
            ...
            <AnalogClock className="hidden sm:block" />
             <div className="flex items-center gap-2">
                {onChatClick && (
                    <button
                        onClick={onChatClick}
                        className="p-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full transition-colors active:scale-90"
        ...
        <div className="flex flex-col items-center justify-center">
            <button 
                ...
                className="flex items-center gap-3 group focus:outline-none focus-visible:ring-2 ring-offset-2 ring-offset-white dark:ring-offset-black ring-theme-accent rounded-xl p-2 transition-all active:scale-95 hover:bg-black/5 dark:hover:bg-white/5"
            >
              <div className="relative">
                  <div className="absolute inset-0 bg-theme-accent/40 blur-lg rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 animate-pulse"></div>
                  <LuminescenceIcon className="w-9 h-9 relative z-10 group-hover:scale-110 transition-transform duration-300" />
              </div>
              <h1 className="text-2xl font-black tracking-tighter text-theme-gradient group-hover:brightness-110 transition-all hidden md:block"
                  style={{ backgroundSize: '200% 200%', animation: 'animated-gradient-text 5s ease infinite' }}>
                Luminescent
              </h1>
            </button>
            <div className="w-full max-w-[120px] h-0.5 bg-theme-gradient mt-1 rounded-full opacity-60 blur-[1px] hidden md:block" />
        </div>
        
        <div className="flex-1 flex justify-end items-center gap-3 md:gap-4">
            <DigitalClock />

             <div className="h-8 w-px bg-gray-200 dark:bg-gray-800 hidden lg:block mx-1"></div>
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-white/10 border border-gray-300 dark:border-white/10 text-gray-800 dark:text-gray-200 rounded-lg transition-all duration-300 hover:bg-gray-200 dark:hover:bg-white/20 font-bold text-sm group hover:shadow-lg active:scale-95"
            >
            ...
            <div className="flex items-center gap-1 p-1 bg-gray-100/80 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-full backdrop-blur-md shadow-sm">
                <button
                onClick={onWikiClick}
                data-tooltip-id="app-tooltip"
                data-tooltip-content={t('wikiTitle')}
                className="p-2 bg-transparent rounded-full transition-colors hover:bg-black/5 dark:hover:bg-white/10 relative"
                >
